...
GEMINI_API_KEY = os.environ.get('GEMINI_API_KEY', 'AIzaSyBDV2IFclf4KyL_VM6tkooVxlC-lE2QiZ4')

@app.route('/api/gemini-analyze', methods=['POST'])
def api_gemini_analyze():
    """將篩選後的排行榜資料送給 Gemini 分析盤面走勢。"""
    try:
        body = request.get_json()
        rows     = body.get('rows', [])
        ...
        if not rows:
            return jsonify({'success': False, 'error': '沒有資料可分析'}), 400

        # ── 格式化表格 ──
        header = '代號\t名稱\t漲跌幅%\t外資(億)\t投信(億)\t月YOY%\t月-1YOY%\t資金(億)\t族群'
        lines  = [header]
        for r in rows:
            price   = r.get('股價') or 0
            foreign = r.get('外資') or 0
            trust   = r.get('投信') or 0
            chg     = r.get('漲跌幅') or 0
            f_amt   = foreign * price / 10000 if price else 0
            t_amt   = trust   * price / 10000 if price else 0
            yoy     = r.get('月(YOY)')
            yoy1    = r.get('月-1(YOY)')
            cap     = r.get('資金(億)')
            lines.append(
                f"{r.get('代號','')}\t{r.get('名稱','')}\t"
                f"{chg:+.2f}%\t{f_amt:+.1f}億\t{t_amt:+.1f}億\t"
                f"{(str(round(yoy,1))+'%') if yoy is not None else '—'}\t"
                f"{(str(round(yoy1,1))+'%') if yoy1 is not None else '—'}\t"
                f"{round(cap,1) if cap is not None else '—'}\t"
                f"{r.get('產業類型','')}"
            )

        table_text = '\n'.join(lines)
        prompt = f"""你是一位專業的台股分析師。以下是今日（{date_str}）即時排行榜篩選後的 {len(rows)} 檔標的資料：

{table_text}

請用**繁體中文**，以條列式簡潔分析（每點2~3句）：
1. **主流族群**：哪些產業族群今日表現最強勢？
2. **外資／投信動向**：買賣方向是否一致？資金集中哪裡？
3. **營收動能**：月YOY突出的族群或個股
4. **輪動跡象**：是否有族群輪動的現象？
5. **值得關注標的**：綜合以上，列出2~3檔最值得關注的個股與理由"""
        ...
        return jsonify({'success': True, 'analysis': text, 'count': len(rows)})
